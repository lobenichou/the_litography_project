            ...
            return $http.get("/api/v1/stories.json").then(
                function(results){
                    ...
                    for (i=0 ; i < data_stories.length; i++){
                        ...
                        if (data_stories[i].book_report == false){
                            for (j=0; j < data_stories[i].locations.length; j++){
                                ...
                                var title = '<div class="popup-content"><h1><em><a href="/stories/' + el.id + '">' + el.title + '</a></em></h1><p>' + el.headline  + '</p><p>By ' + el.author + '</p></div>'
                                ...
                                if (published == true){
                                    if (pub_at == currentMonth){
                                        markers.push({layer: thisMonth, lat:lat, lng:lng, message: content, icon: defaultIcon, visual: el.visual, sound: el.sound, writing: el.writing, multimedia: el.multimedia, id:marker_id })
                                    }else{
                                        markers.push({layer: lastMonth , lat:lat, lng:lng, message: content, icon: defaultIcon, visual: el.visual, sound: el.sound, writing: el.writing, multimedia: el.multimedia, id:marker_id })
                                    }
                                    markers.push({layer: allPast, lat:lat, lng:lng, message: content, icon: defaultIcon, visual: el.visual, sound: el.sound, writing: el.writing, multimedia: el.multimedia, id:marker_id })
                                }
                            }
                        }
                    }
